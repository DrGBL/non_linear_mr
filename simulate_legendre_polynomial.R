...
umx_make_MR_data_legendre <- function(nSubjects = 1000, 
  ...
  #Y variable
  Y <- FUN(X=X,...) + bUY * U + rnorm(nSubjects, 0, sqrt(varY)) # Y variable
  ...
  #add legendre polynomials as columns in MR_data
  legpol<-as.data.frame(polynomial.values(polynomials=legendre.polynomials(pol_order, normalized=TRUE),
                                          x=(2*MR_data$X-min_x-max_x)/(max_x-min_x)))
  colnames(legpol) <- paste0("leg", c(0:pol_order))
  MR_data<-MR_data %>% bind_cols(., legpol)
...
test<-umx_make_MR_data_legendre(nSubjects = 10000, 
...
test<-umx_make_MR_data_legendre(nSubjects = 100000, 
...
test<-umx_make_MR_data_legendre(nSubjects = 50000, 
